function [DT,U,Udot,T,Tdot,time] = stabtrBoussX(nonlin,qmethod,grid,spmat,...
                                       initdata,tfinal,tol,nstar,nmax,tout)
%STABTRBOUSSX Boussinesq integrator using TR (corrected version)
%   [DT,U,Udot,T,Tdot,time] = ...
%   stabtrBoussX(nonlin,qmethod,grid,spmat,initdata,tfinal,tol,nstar,2,1);
%   input
...
%   IFISS function: DJS; 10 May 2012; 24 June 2012; 30 September 2013 .. 19 September 2019
...
%----------------------------------- stabilized version
        if domain==7 & norm(xns,inf)/norm(s,inf) > 1e7; fprintf('oops .. near singularity')
        xnsz=[Jbc,Bbc',zeros(2*nv,1);Bbc,-C,ones(np,1)/np; ...
                        zeros(1,2*nv),ones(1,np)/np,zeros(1,1)]\[fbc+0.5*dt*Mbc*s;gbc;0];
        xns=xnsz(1:end-1); multiplier=xnsz(end); end
